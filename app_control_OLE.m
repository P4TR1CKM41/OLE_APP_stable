...
while strcmp (app.ButtonGroup.SelectedObject.Text, 'Start') && (app.ButtonGroup.SelectedObject.Value ==1)
    ...
    if ~isempty(fileList_cells)
        ...
        if strcmp(choice, 'Yes')
            ...
            try
                [DATA.OPTIONS.ftkratio, DATA.OPTIONS.PATHS.TRC.(trialname), DATA.OPTIONS.PATHS.MOT.(trialname), DATA.CONTACT_ANALOG.(trialname), DATA.CONTACT_KINEMATIC.(trialname), DATA.MARKERS.(trialname), DATA.GRF.(trialname), DATA.OPTIONS.FREQ_KINEMATIC, DATA.OPTIONS.FREQ_ANALOG, DATA.OPTIONS.FP_CORNERS.(trialname), DATA.markers_from_c3d_filt.(trialname),run_time(end+1)] = dynamic_c3d_to_try([app.storageglobal_path, '/', (fileList_cells{1, dy})], app.CutfilesCheckBox.Value,  str2double(app.FPuseDropDown.Value), app.NEditField.Value, DATA.OPTIONS.SETTINGS, DATA.OPTIONS  );
                DATA.OPTIONS.CONVERT_PASSED.(trialname) =1;
            catch ME
                DATA.OPTIONS.CONVERT_PASSED.(trialname) =0;
                DATA.OPTIONS.ERROR_CONVERT.(trialname) = ME;
                buttondlg(['Error when converting ', fileList_cells{1, dy} ,' Check markers and forces in the raw file. Enure that the correct force plate was selected!'],'Error','Ok', struct('Default','Continue','IconString','error'));
            end
            %% Check if foot is on FP
            if app.FoFCheckBox.Value ==1
                try
                    [DATA.OPTIONS.FOOT_IN_FP.(trialname)] = is_foot_on_force_plate_fun(DATA.OPTIONS.MARKER_SETUP, DATA.OPTIONS.SETTINGS, DATA.OPTIONS.FP_CORNERS.(trialname), DATA.CONTACT_KINEMATIC.(trialname)(1),DATA.CONTACT_ANALOG.(trialname)(1), DATA.markers_from_c3d_filt.(trialname),app.Leg2AnalyzeDropDown.Value,0 );
                    if DATA.OPTIONS.FOOT_IN_FP.(trialname)==0
                        buttondlg([fileList_cells{1, dy},' ',app.Leg2AnalyzeDropDown.Value, ' foot was not in the boundaries of force plate number', app.FPuseDropDown.Value ,'!'],'Error','Ok', struct('Default','Continue','IconString','error'));
                    end
                catch ME
                    DATA.OPTIONS.FOOT_IN_FP.(trialname) =0;
                    DATA.OPTIONS.ERROR_FOOT_ON_FORCE_PLATE.(trialname) = ME;
                    buttondlg('Could not determine if foot was placed on force plate. Are all foot markers present and the correct force plate selected?','Error','Ok', struct('Default','Continue','IconString','error'));
                end
            else
                DATA.OPTIONS.FOOT_IN_FP.(trialname) = 1;
            end
            %% check the existens of all necessary marker
            try
                [DATA.OPTIONS.ALL_REQUIRED_MARKERS_EXIST.(trialname), DATA.OPTIONS.MISSING_MARKERS.(trialname)] = check_if_all_markers_are_there(DATA.markers_from_c3d_filt.(trialname), {DATA.OPTIONS.MARKER_SETUP.MARKERSREQUIRED{index_in_marker_set_file_excel,[2:end]}});
                if DATA.OPTIONS.ALL_REQUIRED_MARKERS_EXIST.(trialname) ==0 
                buttondlg([trialname, ' not processed the following markers are missing: ', DATA.OPTIONS.MISSING_MARKERS.(trialname)],'Error','Ok', struct('Default','Continue','IconString','error'));
                end
                catch ME
                DATA.OPTIONS.ERROR_MARKERS.(trialname) = ME;
                DATA.OPTIONS.ALL_REQUIRED_MARKERS_EXIST.(trialname)=0;
                DATA.OPTIONS.MISSING_MARKERS.(trialname) ='all';
            end
            %% start processing OpenSim
            if DATA.OPTIONS.FOOT_IN_FP.(trialname)==1 && DATA.OPTIONS.ALL_REQUIRED_MARKERS_EXIST.(trialname) ==1 && DATA.OPTIONS.CONVERT_PASSED.(trialname) ==1
                ...
                %% ID
                [DATA.OPTIONS.PATHS.ID.(trialname), run_time(end+1)] = ID_app(DATA.OPTIONS.PATHS.MOT.(trialname),DATA.OPTIONS.PATHS.TRC.(trialname),app.FoldertomonitorEditField.Value, DATA.OPTIONS.PATHS.IK.(trialname), DATA.OPTIONS.LEG_ANALYZED.(trialname), DATA.OPTIONS.ForcePlateNumber.(trialname) );
                %% Body kinematics
                [DATA.OPTIONS.PATHS.BK_ACC.(trialname), DATA.OPTIONS.PATHS.BK_VEL.(trialname),DATA.OPTIONS.PATHS.BK_POS.(trialname), run_time(end+1)] = on_Bodykinematics(DATA.OPTIONS.PATHS.IK.(trialname),DATA.OPTIONS.PATH.SCALED_MODEL, app.FoldertomonitorEditField.Value, app.IDEditField.Value, trialname);
