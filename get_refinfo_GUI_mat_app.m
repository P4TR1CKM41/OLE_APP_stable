function [REFFRAME, MARKERS, WEIGHT, OPTIONS] = get_refinfo_GUI_mat_app(ref_file, OPTIONS)
        ...
        try
            [MARKERS.Raw, LABELS.Exist, ~, ~, ~, OPTIONS.FreqKinematics, OPTIONS.ftkratio] = getlabeledmarkers_GUI_mat_app(ref_file);
...
%         try
%             OPTIONS.ANTHRO.FootLength =    abs((nanmean(MARKERS.Filt.calc_back_right.data(1,:)))-(nanmean(MARKERS.Filt.toe_right.data(1,:))));
%         catch
%             OPTIONS.ANTHRO.FootLength = abs((nanmean(MARKERS.Filt.calc_back_left.data(1,:)))-(nanmean(MARKERS.Filt.toe_left.data(1,:))));
%         end
